export type NavLink = {
  label: string;
  href: string;
};

export const nav: {
  brand: string;
  tagline: string;
  links: NavLink[];
  phone: string;
  callback: string;
  cta: { label: string; href: string };
} = {
  brand: "1C Agent Pro",
  tagline: "Мобильное рабочее место торгового представителя",
  links: [
    { label: "О продукте", href: "#about" },
    { label: "Преимущества", href: "#advantages" },
    { label: "Как это работает", href: "#process" },
    { label: "Возможности", href: "#features" },
    { label: "Для кого", href: "#industries" },
    { label: "Внедрение", href: "#implementation" },
    { label: "Тарифы", href: "#pricing" },
    { label: "Контакты", href: "#cta" },
  ],
  phone: "+992 92 864 69 00",
  callback: "Заказать звонок",
  cta: { label: "Оставить заявку", href: "#pricing" },
};

export const hero = {
  eyebrow: "Мобильное рабочее место торгового представителя",
  titleLine1: "Ваши продажи",
  titleLine2: "в",
  titleAccent: "надёжных руках",
  description:
    "Современное решение для эффективной работы торговой команды — заказы, визиты и фотоотчёты с полной синхронизацией с вашей 1С в реальном времени.",
  checklist: [
    "Полная интеграция с 1С",
    "Контроль агентов в реальном времени",
    "Больше визитов. Больше заказов. Больше прибыли.",
  ],
  script: "Больше возможностей для вашего бизнеса!",
  primaryCta: { label: "Узнать подробнее", href: "#pricing" },
  videoButton: { label: "Смотреть демо", href: "#videos" },
  phoneMock: {
    routeLabel: "Маршрут на сегодня",
    stops: [
      { name: "ООО Сино Трейд", time: "09:00" },
      { name: "Маркет 24", time: "10:30" },
      { name: "ИП Каримов", time: "12:00" },
      { name: "Супермаркет Вахдат", time: "14:00" },
    ],
    summaryLabel: "Итого за визит",
    summary: [
      { label: "Заказ создан", value: "1 290 смн" },
      { label: "Фотоотчёт", value: "done" as const },
      { label: "Синхронизация", value: "done" as const },
    ],
  },
  floatingBadges: {
    visits: "4 визита сегодня",
    growth: "+32% продаж",
  },
} as const;

export const stats = [
  { value: "+32%", label: "рост продаж" },
  { value: "−40%", label: "экономия времени" },
  { value: "100%", label: "данные в 1С онлайн" },
  { value: "2 дня", label: "запуск команды" },
] as const;

export const advantages = {
  eyebrow: "Наши преимущества",
  title: "Всё, что нужно для эффективной работы",
  description: "Полный набор возможностей для торгового представителя — в одном решении.",
  script: "Работайте эффективнее каждый день!",
  items: [
    {
      icon: "smartphone",
      image: "/android-logo.svg",
      title: "Работает на Android",
      description: "Удобное мобильное приложение для любой команды.",
    },
    {
      icon: "wifi",
      title: "Онлайн и офлайн режим",
      description: "Работает даже без интернета — все данные сохраняются.",
    },
    {
      icon: "blocks",
      image: "/1c-logo.png",
      title: "Полная интеграция с 1С",
      description: "Заказы, клиенты, остатки и документы автоматически в вашей 1С.",
    },
    {
      icon: "navigation",
      title: "GPS-контроль и маршруты",
      description: "Контроль местоположения, планирование маршрутов и отчёты о визитах.",
    },
    {
      icon: "camera",
      title: "Фотоотчёты и задания",
      description: "Фото точек, мерчендайзинга. Гибкие задания для агента.",
    },
    {
      icon: "rocket",
      title: "Простая установка и быстрый старт",
      description: "Минимальные настройки. Можно начать работу уже за 1 день.",
    },
    {
      icon: "layout-dashboard",
      title: "Удобный и понятный интерфейс",
      description: "Интуитивно понятное приложение, минимум обучения.",
    },
    {
      icon: "trending-up",
      title: "Повышает продажи и контроль",
      description: "Больше визитов, больше заказов, больше прозрачности.",
    },
    {
      icon: "headphones",
      title: "Надёжная поддержка",
      description: "Мы рядом на всех этапах: внедрение, обучение, сопровождение.",
    },
    {
      icon: "gem",
      title: "Подходит для любой отрасли",
      description: "Дистрибуция, опт, производство.",
    },
  ],
} as const;

export const process = {
  eyebrow: "Шаг за шагом",
  title: "Реальный рабочий день агента",
  description:
    "Всё, что нужно для эффективной работы — в одном приложении, от синхронизации утром до отчёта в 1С вечером.",
  videos: [
    { id: "iklYUgCaNnc", title: "Что такое 1C Agent Pro" },
    { id: "w-WSbctvyDs", title: "Рабочий день агента" },
    { id: "AfMRyq_qk4Q", title: "Работа с клиентами" },
    { id: "b7-dx2TnBmc", title: "Установка и подключение" },
  ],
  steps: [
    {
      icon: "refresh-cw",
      title: "Синхронизация",
      duration: "5 минут",
      description:
        "Агент загружает актуальные данные: клиенты, товары, цены, задания и остатки. Работает даже при нестабильном интернете.",
    },
    {
      icon: "route",
      title: "Маршрут",
      duration: "10 минут",
      description:
        "Оптимальный план визитов на день с учётом геолокации. Меньше времени в дороге — больше клиентов.",
    },
    {
      icon: "map-pin",
      title: "Визит к клиенту",
      duration: "15 минут",
      description: "Карточка клиента: история заказов, задолженности, контакты — всё под рукой перед разговором.",
    },
    {
      icon: "shopping-cart",
      title: "Оформление заказа",
      duration: "3 минуты",
      description:
        "Быстрый подбор товаров из каталога с актуальными ценами и остатками. Заказ готов за минуты.",
    },
    {
      icon: "camera",
      title: "Фотоотчёт",
      duration: "2 минуты",
      description:
        "Фото торговой точки и выкладки — подтверждение визита, контроль мерчендайзинга.",
    },
    {
      icon: "check-circle-2",
      title: "Завершение дня",
      duration: "автоматически",
      description:
        "Все заказы, фото и данные визитов синхронизируются в 1С в реальном времени.",
    },
  ],
} as const;

export const features = {
  eyebrow: "Всё необходимое в одном приложении",
  title: "Возможности 1C Agent Pro",
  description:
    "Полный набор инструментов для эффективной работы торгового агента — от базы клиентов до GPS-трека визита.",
  items: [
    {
      icon: "users",
      title: "Клиенты",
      description:
        "База клиентов, контакты, долги по точкам.",
    },
    {
      icon: "package",
      title: "Товары",
      description:
        "Актуальные цены, остатки и фото номенклатуры прямо в карточке товара.",
    },
    {
      icon: "shopping-cart",
      title: "Заказ",
      description:
        "Быстрое оформление заказа: добавление товаров, скидки, автоподбор склада.",
    },
    {
      icon: "map-pin",
      title: "Визиты",
      description:
        "План на день, GPS-контроль, трек маршрута и история фактических визитов.",
    },
    {
      icon: "clipboard-list",
      title: "Задания",
      description:
        "Поручения и плановые задачи от руководителя с контролем сроков выполнения.",
    },
    {
      icon: "camera",
      title: "Фотоотчёт",
      description: "Фото торговых точек, выкладки, мерчендайзинга.",
    },
  ],
} as const;

export const industries = {
  eyebrow: "Специализированное решение",
  title: "Для кого подходит 1C Agent Pro",
  description: "Одно решение для разных моделей полевых продаж.",
  items: [
    {
      icon: "truck",
      title: "Дистрибьюторы",
      points: ["Контроль маршрутов", "Больше покрытия", "Актуальные остатки"],
    },
    {
      icon: "factory",
      title: "Производители",
      points: ["Контроль мерчендайзеров", "Обратная связь с рынка", "Анализ спроса"],
    },
    {
      icon: "users-round",
      title: "Торговые команды",
      points: ["Планирование маршрутов", "Онлайн-обмен с 1С", "Мотивация агентов"],
    },
  ],
} as const;

export const managerControl = {
  eyebrow: "Полный контроль в реальном времени",
  title: "Полный контроль для руководителя",
  description:
    "Вся работа торговой команды — в вашей 1С: прозрачно, наглядно, эффективно.",
  points: [
    {
      icon: "map-pin",
      title: "Геолокация агентов",
      description: "Где находятся сотрудники, каких клиентов посещают",
    },
    {
      icon: "shopping-cart",
      title: "Заказы и продажи",
      description: "Все заказы сразу в 1С, актуальные остатки и цены",
    },
    {
      icon: "bar-chart-3",
      title: "Аналитика и отчёты",
      description: "Показатели по агентам, клиентам, территориям и товарам",
    },
    {
      icon: "navigation",
      title: "Посещения и визиты",
      description: "История визитов по каждому клиенту с фактическим временем",
    },
    {
      icon: "camera",
      title: "Фотоотчёты",
      description: "Фотографии торговых точек и выкладки по каждому визиту",
    },
    {
      icon: "check-circle-2",
      title: "Выполнение заданий",
      description: "Контроль выполнения заданий и плановых показателей агентами",
    },
  ],
  dashboard: {
    title: "Аналитика агента Pro",
    date: "18.09.2026",
    metrics: [
      { label: "Агентов", value: "2" },
      { label: "Заказы, смн", value: "34 000" },
      { label: "Продажи, смн", value: "20 500" },
      { label: "Визитов", value: "32" },
    ],
    agents: [
      { name: "Саидов Б.", status: "online" as const, label: "Онлайн" },
      { name: "Каримов А.", status: "online" as const, label: "В пути" },
      { name: "Ибрагимов С.", status: "busy" as const, label: "Визит" },
      { name: "Насруллаев Д.", status: "online" as const, label: "Онлайн" },
    ],
  },
} as const;

export const pricing = {
  eyebrow: "Тарифы и условия",
  title: "Простая и понятная стоимость",
  description:
    "Выберите подходящий тариф для вашей команды и получайте больше результатов.",
  benefits: [
    { icon: "check-circle-2", label: "Оплата по факту" },
    { icon: "trending-up", label: "Гибкая шкала цен" },
    { icon: "graduation-cap", label: "Внедрение и обучение входят" },
    { icon: "headphones", label: "Поддержка всегда на связи" },
  ],
  tiers: [
    {
      tag: "Standard",
      subtitle: "Базовые мобильные продажи и документы",
      name: "Базовые продажи",
      priceFrom: "от 40",
      unit: "сомони / агент / мес",
      highlighted: false,
      badge: null,
      smsBonusNote: null,
      priceTable: [
        { range: "до 20", price: "50" },
        { range: "21–30", price: "48" },
        { range: "31–40", price: "45" },
        { range: "41–50", price: "43" },
        { range: "от 51+", price: "40" },
      ],
      features: [
        "Клиенты, товары, цены, остатки и штрихкоды",
        "Заказы, продажи, возвраты, покупки, оплаты",
        "Задания, отчёты, журнал документов",
        "Синхронизация с 1С",
      ],
    },
    {
      tag: "Pro",
      subtitle: "Standard плюс фотоотчёт, фотоанализ, визиты, маршруты, геолокация, треки и личная аналитика агента",
      name: "Контроль и аналитика",
      priceFrom: "от 50",
      unit: "сомони / агент / мес",
      highlighted: true,
      badge: "Рекомендуем",
      smsBonusNote:
        "SMS-бонусы: от 500 до 12 000 SMS в зависимости от количества агентов",
      priceTable: [
        { range: "до 20", price: "60", sms: "500 SMS" },
        { range: "21–30", price: "58", sms: "3 000 SMS" },
        { range: "31–40", price: "55", sms: "6 000 SMS" },
        { range: "41–50", price: "53", sms: "9 000 SMS" },
        { range: "от 51+", price: "50", sms: "12 000 SMS" },
      ],
      features: [
        "Все возможности Standard",
        "Фотоотчёт и фотоанализ",
        "Визиты, маршруты, геолокация, треки",
        "Личная аналитика агента",
      ],
    },
    {
      tag: "Business",
      subtitle: "Pro плюс мобильный кабинет руководителя и аналитика визитов, треков и фото по всем агентам",
      name: "Полный контроль",
      priceFrom: "от 60",
      unit: "сомони / агент / мес",
      highlighted: false,
      badge: null,
      smsBonusNote: null,
      priceTable: [
        { range: "до 5", price: "70" },
        { range: "6–10", price: "68" },
        { range: "11–15", price: "65" },
        { range: "16–20", price: "63" },
        { range: "от 21+", price: "60" },
      ],
      features: [
        "Все возможности Pro",
        "Мобильный кабинет руководителя",
        "Аналитика визитов, треков и фото всех агентов",
        "Сводные отчёты и сравнительные показатели",
      ],
    },
  ],
  scaleTable: {
    title: "Шкала цен по количеству агентов",
    subtitle: "Цена за 1 агента в месяц (сомони) — тариф Pro",
    columns: ["до 20", "21–30", "31–40", "41–50", "от 51+"],
    rows: [
      { tier: "Standard", values: ["50", "48", "45", "43", "40"] },
      { tier: "Pro", values: ["60", "58", "55", "53", "50"] },
    ],
  },
  smsBonus: {
    title: "SMS-бонусы",
    subtitle: "только для Pro",
    rows: [
      { agents: "до 20", bonus: "500 SMS" },
      { agents: "21–30", bonus: "3 000 SMS" },
      { agents: "31–40", bonus: "6 000 SMS" },
      { agents: "41–50", bonus: "9 000 SMS" },
      { agents: "от 51+", bonus: "12 000 SMS" },
    ],
  },
  included: [
    { icon: "settings", title: "Настройка и внедрение входят в подписку", description: "Для 1C Agent Pro и SMS" },
    { icon: "graduation-cap", title: "Обучение пользователей и консультации", description: "Без дополнительной платы" },
    { icon: "shield-check", title: "Поддержка и обновления", description: "В течение всего периода подписки" },
    { icon: "file-code", title: "Доработки и шаблоны", description: "В рамках модулей" },
  ],
} as const;

export const implementation = {
  eyebrow: "Внедрение и поддержка",
  title: "Мы не просто даём программу — мы помогаем внедрить",
  steps: [
    {
      icon: "blocks",
      title: "Установим",
      description: "Настроим соединение с вашей 1С — без лишних хлопот.",
    },
    {
      icon: "settings",
      title: "Настроим",
      description: "Параметры системы под ваш бизнес и команду.",
    },
    {
      icon: "graduation-cap",
      title: "Обучим",
      description: "Агентов и руководителей работе в системе.",
    },
    {
      icon: "headphones",
      title: "Поддерживаем",
      description: "На связи в рабочее время — быстро отвечаем на вопросы.",
    },
    {
      icon: "trending-up",
      title: "Развиваем",
      description: "Обновления и доработки включены в подписку.",
    },
  ],
  stats: [
    { value: "до 2 дней", label: "Среднее время внедрения" },
    { value: "1 день", label: "Обучение команды" },
    { value: "1 час", label: "Время реакции поддержки" },
    { value: "Всё включено", label: "в стоимость подписки" },
  ],
  compatibility: {
    label: "Поддерживаемые конфигурации 1С",
    platform: "Платформа 8.3.23 и выше",
    configs: [
      "1С Управление торговлей 11.5",
      "1С Комплексная автоматизация 2.5",
      "1С:ERP Управление предприятием 2.5",
      "1С Управление нашей фирмой 3.0",
      "1С Розница 3.0",
    ],
  },
} as const;

export const valueProps = {
  trial: {
    eyebrow: "Попробуйте бесплатно",
    title: "15 дней без оплаты",
    description:
      "Полный доступ ко всем функциям выбранного тарифа. Никаких скрытых условий — просто попробуйте и убедитесь сами.",
    badge: "Бесплатный период",
    cta: "Начать бесплатно",
    note: "Карта не требуется. Отмена в любой момент.",
  },
  cloud: {
    title: "Бесплатная облачная подписка",
    sections: [
      {
        heading: "Безопасный обмен без статического IP",
        body: "1C Agent Pro обеспечивает обмен данными между 1С и мобильными устройствами агентов без необходимости подключения статического IP-адреса. Данные передаются по зашифрованному соединению.",
      },
      {
        heading: "Балансировка нагрузки и масштабирование",
        body: "Распределение нагрузки и параллельная обработка запросов агентов в 1С позволяют масштабировать 1C Agent Pro по мере роста компании, количества сотрудников и объёма передаваемых данных.",
      },
      {
        heading: "Установка на своём сервере",
        body: "Возможна установка серверного модуля на собственный сервер компании. Статический (белый) IP-адрес не требуется — работает с серым (динамическим) IP.",
      },
    ],
  },
} as const;

export const cta = {
  title: "Готовы навести порядок в полевых продажах?",
  description:
    "Оставьте заявку — подберём тариф, проведём демонстрацию 1C Agent Pro и рассчитаем стоимость для вашей команды.",
  primary: { label: "Оставить заявку", href: "#" },
  secondary: { label: "Написать в WhatsApp", href: "#" },
} as const;

export const footer = {
  brand: "1C Agent Pro",
  tagline: "Мобильное рабочее место торгового представителя",
  phones: ["+992 92 864 69 00", "+992 92 990 94 64"],
  address: "г. Худжанд, ул. Гагарина 137",
  copyright: "© 2026 1C Agent Pro. Все права защищены.",
} as const;

export const playMarketUrl =
  "https://play.google.com/store/apps/details?id=dv.com.agentpro";


export const faq = {
  heading: "Частые вопросы",
  subheading: "Всё, что нужно знать перед внедрением",
  items: [
    {
      q: "С какими конфигурациями 1С совместим 1C Agent Pro?",
      a: "Приложение поддерживает 1С Управление торговлей 11.5+, Комплексная автоматизация 2.5+, ERP Управление предприятием 2.5+, Управление нашей фирмой 3.0+, Розница 3.0+ и 1С Бухгалтерия для Таджикистана 3.0+.",
    },
    {
      q: "Работает ли приложение без интернета?",
      a: "Да. 1C Agent Pro работает в офлайн-режиме: агент принимает заказы, фиксирует визиты и делает фотоотчёты без подключения к сети. Данные синхронизируются с 1С автоматически при следующем выходе в интернет.",
    },
    {
      q: "Сколько времени занимает внедрение?",
      a: "Стандартное внедрение занимает 2 рабочих дня: первый день — установка и настройка интеграции с 1С, второй день — обучение команды и тестовый запуск на маршруте.",
    },
    {
      q: "На каких устройствах работает 1C Agent Pro?",
      a: "Приложение работает на Android-смартфонах и планшетах. Минимальная версия Android — 8.0. Приложение оптимизировано для работы в полевых условиях: яркий экран на улице, работа в перчатках.",
    },
    {
      q: "Сколько стоит 1C Agent Pro?",
      a: "Тарифы начинаются от 40 сомони за агента в месяц (тариф Standard). Цена зависит от количества агентов и выбранного тарифа. Предусмотрен 15-дневный бесплатный пробный период без обязательств.",
    },
    {
      q: "Как контролировать работу агентов в реальном времени?",
      a: "Менеджер видит в веб-панели GPS-треки маршрутов, статус визитов, принятые заказы и фотоотчёты с торговых точек — всё в режиме реального времени. Данные сразу попадают в 1С без ручного ввода.",
    },
  ],
} as const;

export const whatsappPrefill =
  "Здравствуйте! Хочу узнать больше о 1C Agent Pro и получить демо.";

export const demoForm = {
  triggerLabel: "Оставить заявку",
  title: "Оставить заявку",
  subtitle: "Оставьте контакты — перезвоним, ответим на вопросы и проведём демонстрацию.",
  fields: {
    name: { label: "Имя", placeholder: "Как к вам обращаться" },
    phone: { label: "Телефон", placeholder: "+992 __ ___ __ __" },
    config: {
      label: "Конфигурация 1С",
      placeholder: "Выберите конфигурацию",
      options: [
        "1С Управление торговлей 11.5 и новее",
        "1С Комплексная автоматизация 2.5 и новее",
        "1С:ERP Управление предприятием 2.5 и новее",
        "1С Управление нашей фирмой 3.0 и новее",
        "1С Розница 3.0 и новее",
        "1С Бухгалтерия для Таджикистана 3.0 и новее",
        "Другая конфигурация",
      ],
    },
    email: { label: "Email", placeholder: "you@company.com", optional: "необязательно" },
    message: { label: "Сообщение", placeholder: "Кратко опишите вашу задачу или вопрос", optional: "необязательно" },
  },
  submit: "Отправить заявку",
  submitting: "Отправляем…",
  whatsapp: "Написать в WhatsApp",
  success: {
    title: "Заявка отправлена!",
    text: "Мы свяжемся с вами в ближайшее время. Спасибо за интерес к 1C Agent Pro.",
  },
  error: "Не удалось отправить. Попробуйте ещё раз или напишите нам в WhatsApp.",
  consent:
    "Нажимая «Отправить заявку», вы соглашаетесь на обработку персональных данных.",
} as const;
